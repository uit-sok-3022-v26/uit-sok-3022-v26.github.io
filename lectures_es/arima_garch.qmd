...
## Identifying AR and MA structure: the correlogram

Before fitting an ARIMA model, it is necessary to determine the appropriate lag orders $p$ and $q$. The standard diagnostic tool for this is the **correlogram** — a plot of the autocorrelation function (ACF) and the partial autocorrelation function (PACF) of the series $\boldsymbol{\Delta}_d \mathbf{u}$.

The ACF at lag $k$ measures the correlation between $u_t$ and $u_{t-k}$, while the PACF isolates the direct effect of lag $k$ after removing the influence of all intermediate lags. Together, they reveal the fingerprint of the underlying process:

- A pure **AR($p$)** process produces an ACF that decays gradually — either exponentially or in a damped sinusoidal pattern — while the PACF cuts off sharply after lag $p$.
- A pure **MA($q$)** process produces an ACF that cuts off sharply after lag $q$, while the PACF decays gradually.

These contrasting patterns are illustrated in @fig-ar-acf and @fig-ma-acf below.

```{python}
#| label: fig-ar-acf
#| fig-cap: "Correlogram of a simulated AR(1) process with $\\rho_1 = 0.75$. The ACF decays geometrically, while the PACF cuts off after the first lag."
#| echo: false
...
def acf(series, max_lag):
    ...
    return [1.0] + [np.sum((series[k:] - mean) * (series[:-k] - mean)) / (n * c0) for k in range(1, max_lag + 1)]

def pacf(series, max_lag):
    from numpy.linalg import lstsq
    pac = [1.0]
    for k in range(1, max_lag + 1):
        X = np.column_stack([series[k - j: len(series) - j] for j in range(1, k + 1)])
        y = series[k:]
        coef, *_ = lstsq(X, y, rcond=None)
        pac.append(coef[0])
    return pac

acf_vals = np.array(acf(u, max_lag))
pacf_vals = np.array(pacf(u, max_lag))
lags = np.arange(0, max_lag + 1)
ci = 1.96 / np.sqrt(T)

fig, axes = plt.subplots(1, 2, figsize=(9, 3.2))
for ax, vals, title in zip(axes, [acf_vals, pacf_vals], ["ACF", "PACF"]):
    ax.bar(lags, vals, color="#4878CF", width=0.4)
    ax.axhline(ci,  color="red", linestyle="--", linewidth=0.9)
    ax.axhline(-ci, color="red", linestyle="--", linewidth=0.9)
    ax.axhline(0,   color="black", linewidth=0.6)
    ax.set_title(f"AR(1) — {title}", fontsize=11)
    ax.set_xlabel("Lag")
    ax.set_xlim(-0.5, max_lag + 0.5)
    ax.set_ylim(-0.3, 1.05)

plt.tight_layout()
plt.savefig("ar1_correlogram.pdf", bbox_inches="tight")
plt.show()
```

```{python}
#| label: fig-ma-acf
#| fig-cap: "Correlogram of a simulated MA(2) process with $\\lambda_1 = 0.7$ and $\\lambda_2 = 0.4$. The ACF shows significant spikes only at lags 1 and 2 and is negligible thereafter, while the PACF decays gradually."
#| echo: false

lam1, lam2 = 0.7, 0.4

eps2 = rng.normal(0, 1, T + 2)
v = np.array([eps2[t] + lam1 * eps2[t - 1] + lam2 * eps2[t - 2] for t in range(2, T + 2)])

acf_vals2  = np.array(acf(v,  max_lag))
pacf_vals2 = np.array(pacf(v, max_lag))

fig, axes = plt.subplots(1, 2, figsize=(9, 3.2))
for ax, vals, title in zip(axes, [acf_vals2, pacf_vals2], ["ACF", "PACF"]):
    ax.bar(lags, vals, color="#4878CF", width=0.4)
    ax.axhline(ci,  color="red", linestyle="--", linewidth=0.9)
    ax.axhline(-ci, color="red", linestyle="--", linewidth=0.9)
    ax.axhline(0,   color="black", linewidth=0.6)
    ax.set_title(f"MA(2) — {title}", fontsize=11)
    ax.set_xlabel("Lag")
    ax.set_xlim(-0.5, max_lag + 0.5)
    ax.set_ylim(-0.5, 1.05)

plt.tight_layout()
plt.savefig("ma2_correlogram.pdf", bbox_inches="tight")
plt.show()
```

The dashed red lines mark the 95\% confidence band $\pm 1.96/\sqrt{T}$; spikes exceeding these bounds are statistically significant. In practice, a gradual decay in the ACF combined with a sharp cutoff in the PACF after lag $p$ suggests an AR($p$) specification, while the reverse pattern — a sharp ACF cutoff after lag $q$ and a slowly decaying PACF — points to an MA($q$) model. Mixed ARMA processes produce gradual decay in both functions, making the choice of $p$ and $q$ less clear-cut and typically requiring information criteria such as AIC or BIC to guide model selection.
